...
# 0.800164
def run(beta=0.7796, targetBeta=0.7099, nbatch=1, nbatchValidate=1, batch_size=1, nepoch=128,
        cPlaqInit=0.01, cChairInit=0.001, chainLen=1,
    ...
    trainer = forcetrain.Trainer(
        loss=forcetrain.NormLoss(cNormList=[1.0,1.0,1.0], cNormInf=1.0),
    ...
    trainer(confLoader, validateLoader=validateLoader, printWeights=True,
        optChangeEpoch=forcetrain.LRSteps([(nepoch*f,lr) for f,lr in lrSteps]))
