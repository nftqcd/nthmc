...
def run(beta=0.7796, targetBeta=0.7099, nbatch=64, nbatchValidate=1, batch_size=1, nepoch=16,
        dense0Unit=8, shift0=1, dense1Unit=8, shift1=1, dense2Unit=8, shift2=1, dense3Unit=8, shift3=1, dense4Unit=8, denseOut=6,
        chainLen=1,
    ...
    tf.print('shift2:',shift2)
    tf.print('dense3Unit:',dense3Unit)
    tf.print('shift3:',shift3)
    tf.print('dense4Unit:',dense4Unit)
            ...
            transform=transform.TransformChain(
                [transform.StoutSmearSlice(
                    coeff=transform.CoefficientNets([
                        ...
                        transform.Normalization(),
                        transform.SymmetricShifts(symmetric_shifts=shift2),
                        transform.FlattenSiteLocal(input_local_rank=2),

                        tl.Dense(units=dense3Unit, activation='swish'),
                        transform.Normalization(),
                        transform.SymmetricShifts(symmetric_shifts=shift3),
                        transform.FlattenSiteLocal(input_local_rank=2),

                        tl.Dense(units=dense4Unit, activation='swish'),
                        transform.Normalization(),
