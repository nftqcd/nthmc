...
plot \
	...
	"<awk -F '[][,: ]*' '/^topo:/{x=0;for(i=2;i<NF;++i){qq[i]=$i;x+=(qo[i]-qq[i])**2}print x/(NF-2); for(i in qq)qo[i]=qq[i]}' ".file u (steptobeta($0)):1 axes x1y2 @st2 'batch Q^2 diff', \
	"<awk -F '[][,: ]*' '/^topoDiff/{x=0;for(i=2;i<NF;++i){x+=$i}print x/(NF-2)}' ".file u (steptobeta($0)):1 axes x1y2 @st3 'proposed approx Q^2 diff', \
	"<awk -F '[][,: ]*' '/^cosDiff/{x=0;for(i=2;i<NF;++i){x+=$i}print x/(NF-2)}' ".file u (steptobeta($0)):1 @st4 'proposed cosine diff', \
