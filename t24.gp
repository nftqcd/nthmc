...
plot \
	...
	[stepT(0):*] 0.2 @st5 'step length', \
	"<awk -F '[][,: ]*' '/^weights: /{print $2}' ".file u (stepT($0)):(atan($1)/pi) @st6 'coef transform 1', \
	"<awk -F '[][,: ]*' 'w==1{print $2} w>0{w--} /^weights: /{w=11}' ".file u (stepT($0)):(atan($1)/pi) @st7 'coef transform 12', \
