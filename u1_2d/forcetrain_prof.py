...
def train(conf, actionFun, mcmcFun, lossFun, opt, weights=None):
    ...
    profOpt = tf.profiler.experimental.ProfilerOptions(host_tracer_level=2, python_tracer_level=1, device_tracer_level=1, delay_ms=0)
    for epoch in range(conf.nepoch):
        ...
        t0 = tf.timestamp()
        if epoch==1:
            tf.profiler.experimental.start('prof.log/train', options=profOpt)
        for step in range(conf.nstepEpoch):
            if epoch!=1:
        ...
        tf.print('-------- done training epoch', epoch,
            'in', dt, 'sec,', dt/conf.nstepEpoch, 'sec/step --------', summarize=-1)
        if epoch==1:
            tf.profiler.experimental.start('prof.log/infer', options=profOpt)
        ...
        if conf.nstepPostTrain>0:
            ...
            for step in range(conf.nstepPostTrain):
                if epoch!=1:
                    ...
                    with tf.profiler.experimental.Trace("PostTrain/Infer", step_num=step, _r=1):
                        tf.print('# post-training inference step:', step, summarize=-1)
                        x = inferStep(mcmcGen, loss, x, detail=False)
            dt = tf.timestamp()-t0
...
if __name__ == '__main__':
    ...
    conf = nthmc.Conf(nbatch=401, nepoch=3, nstepMixing=1, nstepEpoch=2, nstepPostTrain=2, initDt=0.1, stepPerTraj=7)
    nthmc.setup(conf)
    op0 = (((1,2,-1,-2), (1,-2,-1,2)),
    ...
    actionFun = lambda: nthmc.U1d2(transform(), conf.nbatch, rng.split()[0], size=[54,62])
