...
@tf.function
def trainStep(loss, opt, x, detail=False):
    if detail:
        plaq = loss.action.plaquette(x)
        tf.print('plaq_pre_update:', tf.reduce_mean(plaq), tf.reduce_min(plaq), tf.reduce_max(plaq), summarize=-1)
        tf.print('weights_pre_update:', loss.action.transform.trainable_weights, summarize=-1)
    ...
    if detail:
        tf.print('plaq:', tf.reduce_mean(plaq), tf.reduce_min(plaq), tf.reduce_max(plaq), summarize=-1)
        tf.print('gradients:', grads, summarize=-1)
...
def train(conf, actionFun, mcmcFun, lossFun, opt, weights=None):
    ...
    for epoch in range(conf.nepoch):
        ...
        for step in range(conf.nstepEpoch):
            ...
            trainStep(loss, opt, xtrain, detail=True)
...
if __name__ == '__main__':
    ...
    conf = nthmc.Conf(nbatch=1, nepoch=1, nstepEpoch=1, nstepMixing=1, nstepPostTrain=0, initDt=0.01, stepPerTraj = 4)
    nthmc.setup(conf)
    op0 = (((1,2,-1,-2), (1,-2,-1,2)),
    ...
    actionFun = lambda: nthmc.U1d2(beta=3.0, beta0=2.0, size=(8,8), transform=transform(), nbatch=conf.nbatch, rng=rng.split()[0])
    ...
    run(conf, actionFun, mcmcFun, lossFun, opt)
