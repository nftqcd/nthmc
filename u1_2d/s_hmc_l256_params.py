...
actionFun = lambda: nthmc.U1d2(beta=48.0, beta0=0.0, size=(256,256), transform=ftr.Ident(), nbatch=conf.nbatch, rng=rng.split()[0])
mcmcFun = lambda action: nthmc.Metropolis(conf, evolve.Omelyan2MN(conf, action))

forcetrain.runInfer(conf, actionFun, mcmcFun, saveFile='conf.s_hmc_l256_params')
