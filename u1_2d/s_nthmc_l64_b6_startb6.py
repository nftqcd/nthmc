...
conf = nthmc.Conf(nbatch=32, nepoch=2, nstepEpoch=1024, nstepMixing=128, initDt=trajLength/nstep, stepPerTraj=nstep, trainDt=False, nthr=10, nthrIop=1, seed=7*11*13*7*15*31*1111)
op0 = (((1,2,-1,-2), (1,-2,-1,2)),
...
actionFun = lambda: nthmc.U1d2(beta=7.0, beta0=5.0, size=(64,64), transform=transform(), nbatch=conf.nbatch, rng=rng.split()[0])
mcmcFun = lambda action: nthmc.Metropolis(conf, evolve.Omelyan2MN(conf, action))

x0 = tf.constant(numpy.load('configs/s_hmc_l64_b6_a/conf.b6.0.n1024.l64_64.08192.npy')[0:32], dtype=tf.float64)
forcetrain.runInfer(conf, actionFun, mcmcFun, x0=x0, saveFile='configs/s_nthmc_l64_b6_startb6/conf', transformWeights='weights/t_force_b25_l64_b6')
