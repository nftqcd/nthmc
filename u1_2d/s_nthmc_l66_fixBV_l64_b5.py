...
conf = nthmc.Conf(nbatch=32, nepoch=1, nstepEpoch=1024, nstepMixing=0, initDt=trajLength/nstep, stepPerTraj=nstep, trainDt=False, nthr=10, nthrIop=1, seed=7*11*13*7*15)
op0 = (((1,2,-1,-2), (1,-2,-1,2)),
...
actionFun = lambda: nthmc.U1d2(beta=5.*fac, beta0=0.0, size=(66,66), transform=transform(), nbatch=conf.nbatch, rng=rng.split()[0])
mcmcFun = lambda action: nthmc.Metropolis(conf, evolve.Omelyan2MN(conf, action))

x0 = tf.constant(numpy.load('configs/conf.s_hmc_l66_params.b5.3173828125.n128.l66_66.08192.npy')[0:32], dtype=tf.float64)
forcetrain.runInfer(conf, actionFun, mcmcFun, x0=x0, saveFile='configs/s_nthmc_l66_fixBV_l64_b5/conf', transformWeights='weights/t_force_b25_l64_b5')
