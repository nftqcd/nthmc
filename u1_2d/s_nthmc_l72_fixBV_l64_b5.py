...
actionFun = lambda: nthmc.U1d2(beta=5.*fac, beta0=0.0, size=(72,72), transform=transform(), nbatch=conf.nbatch, rng=rng.split()[0])
mcmcFun = lambda action: nthmc.Metropolis(conf, evolve.Omelyan2MN(conf, action))

x0 = tf.constant(numpy.load('configs/conf.s_hmc_l72_params.b6.328125.n128.l72_72.08192.npy')[0:32], dtype=tf.float64)
forcetrain.runInfer(conf, actionFun, mcmcFun, x0=x0, saveFile='configs/s_nthmc_l72_fixBV_l64_b5/conf', transformWeights='weights/t_force_b25_l64_b5')
