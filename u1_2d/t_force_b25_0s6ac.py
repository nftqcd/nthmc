...
conf = nthmc.Conf(nbatch=2, nepoch=4, nstepEpoch=1024, nstepMixing=128, nstepPostTrain=1024, initDt=0.2, stepPerTraj=10, nthr=20, nthrIop=2, seed=1234567)
nthmc.setup(conf)
op0 = (((1,2,-1,-2), (1,-2,-1,2)),
...
convP0 = lambda: ftr.PeriodicConv((
    tk.layers.Conv2D(3, (3,2), activation='gelu', kernel_initializer=tk.initializers.RandomNormal(), bias_initializer=tk.initializers.RandomNormal()),
))
convP1 = lambda: ftr.PeriodicConv((
    tk.layers.Conv2D(3, (2,3), activation='gelu', kernel_initializer=tk.initializers.RandomNormal(), bias_initializer=tk.initializers.RandomNormal()),
))
convR = lambda pad: ftr.PeriodicConv((
    tk.layers.Conv2D(3, (3,3), activation='gelu', kernel_initializer=tk.initializers.RandomNormal(), bias_initializer=tk.initializers.RandomNormal()),
), pad)
...
actionFun = lambda: nthmc.U1d2(beta=6.0, beta0=2.0, size=(128,128), transform=transform(), nbatch=conf.nbatch, rng=rng.split()[0])
loss = lambda action: forcetrain.LossFun(action, betaMap=2.5, cNorm2=1.0, cNormInf=1.0)
opt = tk.optimizers.Adam(learning_rate=0.008)
