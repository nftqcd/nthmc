...
conf = nthmc.Conf(nbatch=128, nepoch=1, nstepEpoch=1024, nstepMixing=128, nstepPostTrain=1024, initDt=0.2, stepPerTraj=10, nthr=20, nthrIop=2, seed=12*45*7)
nthmc.setup(conf)
op0 = (((1,2,-1,-2), (1,-2,-1,2)),
...
actionFun = lambda: nthmc.U1d2(beta=3.0, beta0=3.0, size=(64,64), transform=transform(), nbatch=conf.nbatch, rng=rng.split()[0])
...
loadConf = lambda n: tf.constant(numpy.load(f'configs/s_hmc_l64_b3/conf.b3.0.n128.l64_64.{16*(n+1):05d}.npy', allow_pickle=False, fix_imports=False), dtype=tf.float64)
forcetrain.trainWithConfs(conf, loadConf, actionFun, loss, opt, saveWeights='weights/t_force_b25_l64_b3')
