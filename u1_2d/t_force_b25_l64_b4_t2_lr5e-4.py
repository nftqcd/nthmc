...
conf = nthmc.Conf(nbatch=128, nepoch=1, nstepEpoch=2048, nstepMixing=128, nstepPostTrain=1024, initDt=0.2, stepPerTraj=10, nthr=20, nthrIop=2, seed=1234567)
nthmc.setup(conf)
op0 = (((1,2,-1,-2), (1,-2,-1,2)),
...
actionFun = lambda: nthmc.U1d2(beta=4.0, beta0=4.0, size=(64,64), transform=transform(), nbatch=conf.nbatch, rng=rng.split()[0])
loss = lambda action: forcetrain.LossFun(action, betaMap=2.5, cNorm2=1.0, cNorm4=1.0, cNorm6=1.0, cNormInf=1.0)
opt = tk.optimizers.Adam(learning_rate=5e-4)
mcmc = lambda action: nthmc.Metropolis(conf, evolve.Omelyan2MN(conf, action))
loadConf = lambda n: tf.constant(numpy.load(f'configs/s_hmc_l64_b4/conf.b4.0.n128.l64_64.{512*(n%1024+1):05d}.npy', allow_pickle=False, fix_imports=False), dtype=tf.float64)
forcetrain.trainWithConfs(conf, loadConf, actionFun, loss, opt, loadWeights='weights/t_force_b25_l64_b3', saveWeights='weights/t_force_b25_l64_b4_t2_lr5e-4')
