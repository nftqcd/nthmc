...
actionFun = lambda: nthmc.U1d2(beta=5.0, beta0=5.0, size=(64,64), transform=transform(), nbatch=conf.nbatch, rng=rng.split()[0])
loss = lambda action: forcetrain.LossFun(action, betaMap=2.5, cNorm2=1.0, cNorm4=1.0, cNorm6=5.0, cNorm8=5.0, cNorm10=0.0, cNormInf=1.0)
opt = tk.optimizers.Adam(learning_rate=0.000005)
mcmc = lambda action: nthmc.Metropolis(conf, evolve.Omelyan2MN(conf, action))
def loadConf(n):
    # n from 0 to 1023
    ab = 'a' if n%2==0 else 'b'
    k = n//2
    i = k//8
    j = k%8
    return tf.constant(numpy.load(f'configs/s_hmc_l64_b5_{ab}/conf.b5.0.n1024.l64_64.{2048*(i+1):05d}.npy', allow_pickle=False, fix_imports=False)[128*j:128*(j+1)], dtype=tf.float64)
forcetrain.trainWithConfs(conf, loadConf, actionFun, loss, opt, loadWeights='weights/t_force_b25_l64_b4_cn8', saveWeights='weights/t_force_b25_l64_b5_step5e-5_p7o16_5cn6_5cn8', optChange=forcetrain.LRSteps([(conf.nstepEpoch*7/16, 0.00005), (conf.nstepEpoch*7/8, 0.000005), (conf.nstepEpoch, 0.00000005)]))
