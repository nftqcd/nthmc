...
plot \
	...
	"<awk -F '[][,: ]*' '/^weights/{print $2}' ".file u (steptobetaT($0)):1 @st5 'step length', \
	"<awk -F '[][,: ]*' 'w==1{print $2} w>0{w--} /^weights/{w=1}' ".file u (steptobetaT($0)):(atan($1)/pi) @st6 'coef transform 1', \
	"<awk -F '[][,: ]*' 'w==1{print $2} w>0{w--} /^weights/{w=12}' ".file u (steptobetaT($0)):(atan($1)/pi) @st7 'coef transform 12', \
